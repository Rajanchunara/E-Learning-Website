...
function Success() {
  ...
  try {
    const decodedInfo = atob(info); // decode base64
    newInfo = JSON.parse(decodedInfo); // parse JSON
    console.log("Decoded Info:", decodedInfo);
    console.log("Parsed Info:", newInfo);
  } catch (error) {
    console.error("Failed to decode or parse info:", error);
  }
